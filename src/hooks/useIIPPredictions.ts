...
export interface IIPPredictionData {
  narrative: string;
  forecasts: {
    index: number[]; // Projected IIP index values
    growthRate: number[]; // Projected growth rates (%)
    years: number[]; // Corresponding years
  };
  kpis: {
    projectedIndex: number; // Projected IIP index for target year
...
export interface IIPHistoricalData {
  category: string;
...
export async function generateIIPPredictions(
  data: IIPHistoricalData
): Promise<IIPPredictionData> {
  const {
    category,
    historicalData,
    latestIndex,
  ...
  dataPoints.push(`IIP Category: ${category}`);
  dataPoints.push(`Historical data (last 10 years): ${dataSummary}`);
  if (latestIndex != null) dataPoints.push(`Latest IIP Index: ${latestIndex.toFixed(1)}`);
  ...
  const systemContent = `You are an Industrial Production Index (IIP) Forecasting Analyst specializing in predictive analytics for industrial growth and production indices.

Your task is to generate accurate, data-driven predictions for IIP index and growth rates based on historical trends.
...
{
  "narrative": "A concise explanation (150-200 words) of future industrial production trends, key drivers, and what to expect${targetYear ? `, with specific focus on ${targetYear}` : ''}",
  "forecasts": {
    "index": [number, number, ...], // Projected IIP index values${targetYear ? ` starting from ${startYear} and extending to at least ${targetYear} (include ${targetYear} in the array)` : ' for next 5-10 years'}
  ...
  "kpis": {
    "projectedIndex": number, // Projected IIP index${targetYear ? ` for ${targetYear}` : ' for next year'}
...
- Index, growthRate, and years arrays must have the same length
- Index values should be consistent with historical ranges
- Growth rates should be realistic (typically -10% to +20% range for IIP)
...
- Ensure no trailing commas in arrays or objects`;
  ...
  const prompt = `Based on the following IIP data for ${category}, generate predictions:
...
Output the result as a JSON object matching the specified structure.`;
    ...
    if (isAuthError) {
      ...
      throw new Error(
        isInvalidKey
          ...
          : "IIP predictions are not configured. Add OPENAI_API_KEY_admin to your .env file and restart the dev server."
      );
    }
  ...
  try {
    ...
    let parsed: IIPPredictionData;
    try {
      parsed = JSON.parse(jsonContent) as IIPPredictionData;
    } catch (parseError) {
    ...
    if (!parsed.narrative || !parsed.forecasts || !parsed.forecasts.index || !parsed.forecasts.growthRate || !parsed.forecasts.years) {
    ...
    try {
      ...
      const retryParsed = JSON.parse(cleanedContent) as IIPPredictionData;
...
export function useIIPPredictions() {
  ...
  const generatePredictions = async (data: IIPHistoricalData) => {
    ...
    try {
      ...
      setError(null);
      const result = await generateIIPPredictions(data);
