...
/** Numeric avg yield (tons/ha) for profitability calculations. */
export function getYieldForStateCrop(stateId: string, cropKey: "rice" | "wheat"): number {
  const raw = CROP_BY_STATE[stateId];
  if (!raw) return 0;
  return cropKey === "rice" ? raw.rice.avgYield : raw.wheat.avgYield;
}
...
export function getStatewiseProduction(): StatewiseProductionItem[] {
  const order = [
    "West Bengal", "Uttar Pradesh", "Punjab", "Andhra Pradesh", "Telangana", "Odisha",
    "Chhattisgarh", "Tamil Nadu", "Bihar", "Madhya Pradesh", "Haryana", "Rajasthan", "Gujarat", "Maharashtra",
  ];
