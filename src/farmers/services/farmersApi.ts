/**
 * Farmers API: weather, disease detection.
 */
...
export interface WeatherForecast {
  location: string;
  current: { temp: number; humidity: number; windSpeed: number; weatherCode: number };
  daily: Array<{ date: string; tempMax: number; tempMin: number; weatherCode: number }>;
}

const WMO_CODES: Record<number, string> = {
  0: "Clear Sky", 1: "Mainly Clear", 2: "Partly Cloudy", 3: "Overcast",
  45: "Foggy", 48: "Depositing Rime Fog", 51: "Light Drizzle", 61: "Slight Rain",
  63: "Moderate Rain", 80: "Slight Rain Showers", 95: "Thunderstorm", 96: "Thunderstorm with Hail",
};
...
export async function fetchGeocode(cityName: string): Promise<GeocodeResult | null> {
  ...
  const r = results[0];
  return { name: r.name ?? cityName, latitude: r.latitude, longitude: r.longitude, country_code: r.country_code ?? "IN", admin1: r.admin1 };
}
...
export async function fetchWeatherByCoords(lat: number, lon: number): Promise<Omit<WeatherForecast, "location">> {
  const params = new URLSearchParams({
    ...
    daily: "weather_code,temperature_2m_max,temperature_2m_min", timezone: TZ, forecast_days: "3",
  });
  ...
  const dailyList: WeatherForecast["daily"] = (daily?.time ?? []).slice(0, 3).map((_: string, i: number) => ({
    date: daily.time[i], tempMax: daily.temperature_2m_max?.[i] ?? 0, tempMin: daily.temperature_2m_min?.[i] ?? 0, weatherCode: daily.weather_code?.[i] ?? 0,
  }));
  return {
    current: {
      temp: current?.temperature_2m ?? 0, humidity: current?.relative_humidity_2m ?? 0,
      windSpeed: current?.wind_speed_10m ?? 0, weatherCode: current?.weather_code ?? 0,
    },
    daily: dailyList,
  };
}
...
export interface DiagnosisItem { name: string; confidence: number; description?: string | null; }
export interface DiagnosisResponse {
  ...
  diagnosis?: {
    ...
    fertilizersOrPesticides?: string[];
  };
  disclaimer?: string;
  error?: string;
}
